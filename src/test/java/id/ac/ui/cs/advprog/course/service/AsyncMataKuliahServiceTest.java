...
class AsyncMataKuliahServiceTest {
    ...
    @Mock
    private MataKuliahService mockMataKuliahService;
    ...
    void setUp() {
        ...
        reset(mockMataKuliahService);

        // Inject mock ke service menggunakan reflection
        try {
            var field = AsyncMataKuliahService.class.getDeclaredField("mataKuliahService");
            field.setAccessible(true);
            field.set(asyncMataKuliahService, mockMataKuliahService);
        } catch (Exception e) {
    ...
    void findAllAsync_ShouldReturnCoursesAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        when(mockMataKuliahService.findAll(any(Pageable.class))).thenReturn(mockPage);
        ...
        verify(mockMataKuliahService, times(1)).findAll(any(Pageable.class));
    }

    @Test
    void findByKodeAsync_ShouldReturnSpecificCourse() throws ExecutionException, InterruptedException, TimeoutException {
        // Arrange
        when(mockMataKuliahService.findByKode("CS101")).thenReturn(sampleCourse);
        ...
        verify(mockMataKuliahService, times(1)).findByKode("CS101");
    }

    @Test
    void findByKodeAsync_ShouldHandleException() {
        // Arrange
        when(mockMataKuliahService.findByKode("INVALID")).thenThrow(new RuntimeException("Course not found"));
        ...
        verify(mockMataKuliahService, times(1)).findByKode("INVALID");
    }
    ...
    void createAsync_ShouldCreateCourseAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        // Arrange
        when(mockMataKuliahService.create(any(MataKuliahDto.class))).thenReturn(sampleCourse);
        ...
        verify(mockMataKuliahService, times(1)).create(any(MataKuliahDto.class));
    }
    ...
    void updateAsync_ShouldUpdateCourseAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        when(mockMataKuliahService.update(eq("CS101"), any(MataKuliahDto.class))).thenReturn(updatedCourse);
        ...
        verify(mockMataKuliahService, times(1)).update(eq("CS101"), any(MataKuliahDto.class));
    }
    ...
    void partialUpdateAsync_ShouldApplyPatchAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        MataKuliahDto patchedCourse = new MataKuliahDto(
                "CS101",
                "Introduction to Computer Science",
                4, // Updated SKS from patch
                "Updated description", // Updated description from patch
                List.of(UUID.randomUUID())
        );
        when(mockMataKuliahService.partialUpdate(eq("CS101"), any(MataKuliahPatch.class))).thenReturn(patchedCourse);
        ...
        verify(mockMataKuliahService, times(1)).partialUpdate(eq("CS101"), any(MataKuliahPatch.class));
    }

    @Test
    void deleteAsync_ShouldDeleteCourseAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        // Arrange
        doNothing().when(mockMataKuliahService).delete("CS101");
        ...
        assertThat(result).isNull(); // Void should be null

        verify(mockMataKuliahService, times(1)).delete("CS101");
    }
    ...
    void addLecturerAsync_ShouldAddLecturerAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        doNothing().when(mockMataKuliahService).addLecturer("CS101", lecturerId);
        ...
        verify(mockMataKuliahService, times(1)).addLecturer("CS101", lecturerId);
    }
    ...
    void removeLecturerAsync_ShouldRemoveLecturerAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        // Arrange
        UUID lecturerId = UUID.randomUUID();
        doNothing().when(mockMataKuliahService).removeLecturer("CS101", lecturerId);
        ...
        verify(mockMataKuliahService, times(1)).removeLecturer("CS101", lecturerId);
    }
    ...
    void createMultipleAsync_ShouldCreateMultipleCoursesAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        when(mockMataKuliahService.create(any(MataKuliahDto.class)))
                .thenReturn(course1)
                .thenReturn(course2);
        ...
        assertThat(result.get(1).kode()).isEqualTo("CS102");

        verify(mockMataKuliahService, times(2)).create(any(MataKuliahDto.class));
    }
    ...
    void searchCoursesAsync_ShouldPerformSearchAsynchronously() throws ExecutionException, InterruptedException, TimeoutException {
        ...
        when(mockMataKuliahService.findAll(any(Pageable.class))).thenReturn(allCourses);
        ...
        assertThat(result).isNotNull();
        assertThat(result).hasSize(1); // Only "Computer Science" should match
        assertThat(result.get(0).kode()).isEqualTo("CS101");
    ...
    void asyncOperations_ShouldExecuteConcurrently() throws ExecutionException, InterruptedException, TimeoutException {
        // Arrange
        when(mockMataKuliahService.findByKode(anyString())).thenReturn(sampleCourse);
        ...
        // Assert
        // Should execute concurrently, so total time should be much less than sequential
        // Each operation has ~200ms delay, so sequential would be ~600ms
        // Concurrent should be around ~200ms + overhead
        assertThat(executionTime).isLessThan(500);

        // Verify all operations were called
        verify(mockMataKuliahService, times(3)).findByKode(anyString());
    }

    @Test
    void asyncOperations_ShouldHandleExceptionsGracefully() {
        // Arrange
        when(mockMataKuliahService.findByKode("VALID")).thenReturn(sampleCourse);
        when(mockMataKuliahService.findByKode("INVALID")).thenThrow(new RuntimeException("Not found"));
